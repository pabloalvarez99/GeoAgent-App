...
// Firestore collection name differs from the URL segment
const COL: &str = "structural_data";
...
pub async fn list(
    ...
    let filter = params.get("stationId").map(|v| ("stationId", v.as_str()));
...
pub async fn update(State(fs): State<FirestoreClient>, auth: AuthUser, Path(id): Path<String>, Json(mut body): Json<Value>) -> Result<Json<Value>, AppError> {
    stamp(&mut body);
    Ok(Json(fs.set(&auth.token, &auth.uid, COL, &id, &body).await.map_err(|e| e.into_app_err())?))
}
