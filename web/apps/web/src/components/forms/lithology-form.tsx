  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="space-y-4">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Tipo de roca</p>
        <div className="grid grid-cols-2 gap-4">
          <GeoSelect label="Grupo de roca" required options={ROCK_GROUPS} value={watch('rockGroup')} onChange={(v) => { setValue('rockGroup', v); setValue('rockType', ''); }} error={errors.rockGroup?.message} />
          <GeoSelect label="Tipo de roca" required options={rockTypes} value={watch('rockType')} onChange={(v) => setValue('rockType', v)} error={errors.rockType?.message} placeholder={rockGroup ? 'Seleccionar tipo' : 'Selecciona grupo primero'} />
        </div>
        <div className="grid grid-cols-2 gap-4">
