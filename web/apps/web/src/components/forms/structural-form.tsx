  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="space-y-1.5">
          <Label>Rumbo (°) *</Label>
          <Input type="number" min="0" max="360" placeholder="0–360" {...register('strike', { valueAsNumber: true })} />
          {errors.strike && <p className="text-xs text-destructive">{errors.strike.message}</p>}
        </div>
        <div className="space-y-1.5">
          <Label>Buzamiento (°) *</Label>
          <Input type="number" min="0" max="90" placeholder="0–90" {...register('dip', { valueAsNumber: true })} />
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input type="number" step="0.01" min="0" placeholder="0.00" {...register('thickness', { valueAsNumber: true, setValueAs: (v) => v === '' ? null : Number(v) })} />
        </div>
      </div>
