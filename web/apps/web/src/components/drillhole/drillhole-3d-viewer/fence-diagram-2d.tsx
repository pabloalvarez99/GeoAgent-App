...
import type { SectionAxis, SectionRibbon } from './section-plane';
...
interface PanelData {
  ...
  collars: Array<{ u: number; v: number; label: string }>;
...
function projectAxis(
  ...
  if (axis === 'ns') return [cz, cy, cx];
  if (axis === 'ew') return [cx, cy, cz];
  return [cx, cz, cy];
}

function buildPanel(flat: FlatInstance[], rb: SectionRibbon, defaultThickness: number): PanelData {
  const segments = flat.map((f) => {
    ...
    const [u1, v1, perp1] = projectAxis(rb.axis, sx, sy, sz);
    const [u2, v2, perp2] = projectAxis(rb.axis, ex, ey, ez);
    const d1 = perp1 - rb.depth;
    const d2 = perp2 - rb.depth;
    const half = defaultThickness > 0 ? defaultThickness / 2 : Infinity;
    const inSlab = defaultThickness > 0 ? Math.min(Math.abs(d1), Math.abs(d2)) <= half : true;
    return {
      u1, v1, u2, v2,
      color: f.color,
      holeId: f.hole.id,
  ...
  const collarsMap = new Map<string, { u: number; v: number; label: string }>();
  flat.forEach((f) => {
    if (collarsMap.has(f.hole.id)) return;
    const [u, v] = projectAxis(rb.axis, f.collar.x, f.collar.y, f.collar.z);
  ...
  const bounds = { umin: umin - padU, umax: umax + padU, vmin: vmin - padV, vmax: vmax + padV };
...
export function FenceDiagram2D({ flat, ribbons, thickness, onClose, projectId }: Props) {
  const panels = useMemo(
    () => ribbons.map((rb) => buildPanel(flat, rb, thickness)),
